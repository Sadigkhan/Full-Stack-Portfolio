...
namespace DekorEvStartUpFinal.Areas.Manage.Controllers
{
    ...
    public class RemovableVIPController : Controller
    {
        private readonly DekorEvStartupAppDbContext _context;
        private readonly IWebHostEnvironment _env;

        public RemovableVIPController(DekorEvStartupAppDbContext context, IWebHostEnvironment env)
        {
            _context = context;
            _env = env;
        }
        ...
        public async Task<IActionResult> Index( int page = 1)
        {
            ...
            IEnumerable<Product> products = await _context.Products
             .Where(p => p.AppUser.NormalizedUserName != User.Identity.Name.ToUpperInvariant() &&p.IsVip&&p.IsFronted&& !p.IsPremium&&!p.IsDeleted)
        ...
        public async Task<IActionResult> RemoveVip(int? id, int page = 1)
        {
            ...
            IEnumerable<Product> products = await _context.Products
                 .Where(p => p.AppUser.NormalizedUserName != User.Identity.Name.ToUpperInvariant() && p.IsVip && !p.IsDeleted)
                .Include(p => p.Category)
            ...
            return PartialView("_VipIndexPartial", products.Skip((page - 1) * 5).Take(5));
        }
    }
}
