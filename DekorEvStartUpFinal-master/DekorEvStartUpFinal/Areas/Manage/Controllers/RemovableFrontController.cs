...
namespace DekorEvStartUpFinal.Areas.Manage.Controllers
{
    ...
    public class RemovableFrontController : Controller
    {
        private readonly DekorEvStartupAppDbContext _context;
        private readonly IWebHostEnvironment _env;

        public RemovableFrontController(DekorEvStartupAppDbContext context, IWebHostEnvironment env)
        {
            _context = context;
            _env = env;
        }
        public async Task<IActionResult> Index(int page = 1)
        {
            ...
            IEnumerable<Product> products = await _context.Products
             .Where(p => p.AppUser.NormalizedUserName != User.Identity.Name.ToUpperInvariant() && p.IsFronted &&!p.IsVip && !p.IsPremium && !p.IsDeleted)
        ...
        public async Task<IActionResult> RemoveFront(int? id, int page = 1)
        {
            ...
            return PartialView("_FrontIndexPartial", products.Skip((page - 1) * 5).Take(5));
        }
    }
}
